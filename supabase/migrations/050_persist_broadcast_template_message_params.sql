-- ============================================================
-- 050_persist_broadcast_template_message_params.sql
--
-- A media-header broadcast can override the template's stored sample media URL
-- at send time. The browser used that override for the initial fan-out but did
-- not persist it, so Resume/Retry could silently fall back to a different
-- template.header_media_url. Persist campaign-wide structured send-time params
-- atomically with campaign creation.
-- ============================================================

ALTER TABLE broadcasts
  ADD COLUMN IF NOT EXISTS template_message_params JSONB NOT NULL DEFAULT '{}'::jsonb;

-- Replacing the signature rather than leaving an overload prevents privileged
-- callers from creating a resumable campaign without freezing its send-time
-- parameters. Migration 047 established this same "one canonical overload"
-- rule for the older channel-blind signatures.
...
CREATE OR REPLACE FUNCTION public.create_broadcast_with_recipients(
  ...
  p_template_message_params JSONB
)
...
BEGIN
  ...
  IF p_template_message_params IS NULL
     OR jsonb_typeof(p_template_message_params) IS DISTINCT FROM 'object' THEN
    RAISE EXCEPTION 'Broadcast template_message_params must be a JSON object'
      USING ERRCODE = '22023';
  END IF;
  ...
  INSERT INTO broadcasts (
    ...
    template_language, template_message_params, status, total_recipients
  )
  VALUES (
    p_account_id, p_user_id, p_whatsapp_config_id, p_name, p_template_name,
    p_template_language, p_template_message_params, 'sending', p_total_recipients
  )
...
REVOKE ALL ON FUNCTION public.create_broadcast_with_recipients(
  UUID, UUID, TEXT, TEXT, TEXT, INTEGER, UUID[], JSONB[], UUID, JSONB
) FROM PUBLIC;
REVOKE ALL ON FUNCTION public.create_broadcast_with_recipients(
  UUID, UUID, TEXT, TEXT, TEXT, INTEGER, UUID[], JSONB[], UUID, JSONB
) FROM anon;

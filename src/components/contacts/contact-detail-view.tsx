...
export function ContactDetailView({
  ...
  // with this contact by sending an approved template. In a multi-number
  // workspace the agent must have an explicit From channel; the selected
  // channel is also used to scope the template catalog.
  ...
  useEffect(() => {
    ...
    void fetch('/api/whatsapp/config?list=1', { cache: 'no-store' })
      .then(async (res) => {
        ...
        if (cancelled) return;
        const channels = (Array.isArray(json.channels) ? json.channels : []) as WhatsAppChannel[];
        setWhatsappChannels(channels);
        setTemplateChannelId((current) => {
          if (current && channels.some((channel) => channel.id === current)) return current;
          return channels.find((channel) => channel.is_primary)?.id ?? channels[0]?.id ?? '';
        });
      })
      .catch((error) => {
        console.error('Failed to load WhatsApp channels for contact send:', error);
        if (!cancelled) {
          setWhatsappChannels([]);
  ...
  async function handleSendTemplate(
    ...
    const selectedChannelId = template.whatsapp_config_id ?? templateChannelId;
    if (!selectedChannelId) {
      toast.error('Choose a WhatsApp number before sending.');
      return;
    }
    if (
      template.whatsapp_config_id &&
      templateChannelId &&
      template.whatsapp_config_id !== templateChannelId
    ) {
      toast.error('Selected template does not belong to the chosen WhatsApp number.');
      return;
    }
  ...
  const selectedWhatsappChannel = whatsappChannels.find(
    (channel) => channel.id === templateChannelId,
  );
  ...
  return (
    <>
              ...
              <SheetHeader className="p-4 border-b border-border/50">
                ...
                <div className="mt-3 flex flex-col gap-2 sm:flex-row sm:items-center">
                  {whatsappChannels.length > 0 && (
                    <label className="flex min-w-0 flex-1 items-center gap-2 rounded-md border border-border bg-background px-2.5 py-1.5 text-xs text-muted-foreground">
                      ...
                      <select
                        ...
                        onChange={(event) => {
                          ...
                          setTemplateChannelId(event.target.value);
                        }}
                        className="min-w-0 flex-1 bg-transparent text-foreground outline-none"
                        aria-label="WhatsApp sending number"
                      >
                  ...
                  <Button
                    ...
                    disabled={sendingTemplate || !templateChannelId}
                    className="bg-primary text-primary-foreground hover:bg-primary/90"
                    title={
                      selectedWhatsappChannel
                        ? `Send from ${selectedWhatsappChannel.label || selectedWhatsappChannel.phone_number_id}`
                        : 'Connect a WhatsApp number before sending'
                    }
                  >
                    {sendingTemplate ? (
      ...
      <TemplatePicker
        ...
        channelId={templateChannelId || null}
      />
    </>
  );
}

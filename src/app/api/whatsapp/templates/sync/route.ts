...
function parseButtons(metaButtons: MetaButton[] | undefined): TemplateButton[] {
  ...
  for (const b of metaButtons) {
    switch (b.type?.toUpperCase()) {
      case 'QUICK_REPLY': out.push({ type: 'QUICK_REPLY', text: b.text }); break
      case 'URL': out.push({ type: 'URL', text: b.text, url: b.url ?? '', example: Array.isArray(b.example) ? b.example[0] : b.example }); break
      case 'PHONE_NUMBER': out.push({ type: 'PHONE_NUMBER', text: b.text, phone_number: b.phone_number ?? '' }); break
      case 'COPY_CODE': out.push({ type: 'COPY_CODE', text: b.text, example: Array.isArray(b.example) ? b.example[0] ?? '' : b.example ?? '' }); break
    }
  }
  return out
}
...
export async function POST(request: Request) {
  try {
    ...
    if (requestedChannelId) {
      const result = await supabase.from('whatsapp_config').select('*').eq('account_id', accountId).eq('id', requestedChannelId).maybeSingle()
      config = result.data
    ...
    if (!config.waba_id) return NextResponse.json({ error: 'WABA ID missing for this channel.' }, { status: 400 })
    ...
    while (nextUrl && pageCount < PAGE_CAP) {
      ...
      if (!metaRes.ok) {
        ...
        try { const body = await metaRes.json(); if (body?.error?.message) metaErr = body.error.message } catch {}
    ...
    for (const t of metaTemplates) {
      ...
      const headerType = ['TEXT', 'IMAGE', 'VIDEO', 'DOCUMENT'].includes(headerFormat ?? '') ? headerFormat!.toLowerCase() : null
      const row = {
      ...
      const { data: existing, error: lookupErr } = await supabase.from('message_templates').select('id').eq('whatsapp_config_id', config.id).eq('name', t.name).eq('language', t.language).maybeSingle()
      if (lookupErr) { errors.push({ name: t.name, language: t.language, message: lookupErr.message }); continue }
      if (existing?.id) {
        const { error } = await supabase.from('message_templates').update(row).eq('id', existing.id)
        if (error) errors.push({ name: t.name, language: t.language, message: error.message }); else updated++
      } else {
        const { error } = await supabase.from('message_templates').insert(row)
        if (error) errors.push({ name: t.name, language: t.language, message: error.message }); else inserted++
      }
    }

    return NextResponse.json({ success: errors.length === 0, channel_id: config.id, total: metaTemplates.length, inserted, updated, errors, truncated: pageCount >= PAGE_CAP && nextUrl !== null })
  } catch (error) {
